...
•	SGsongssortedUser... .txt (for each user)
•	SG User toVec ... .txt  (for each user)
Printed outputs:
...
for user in range(df.shape[0]):
    ...
    saveName = fileLocation + 'SGsongssortedUser ' + str(user) + " " + modelName + '.txt'
    with open(saveName, "wb") as myFile:
        pickle.dump(songsSorted, myFile)

    saveName = fileLocation + 'SGsong2similarityDictionaryUser ' + str(user) + " " + modelName + '.txt'
    with open(saveName, "wb") as myFile:
        pickle.dump(songSimDic, myFile)

    saveName = fileLocation + 'SG User' + str(user) + 'toVec ' + modelName + '.txt'
    with open(saveName, "wb") as myFile:
